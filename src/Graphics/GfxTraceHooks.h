...
public:
	...
	virtual void UpdateBuffer( BufferHandle buf, const void * data_ptr, int data_size );
	virtual void UpdateImage( ImageHandle img, const ImageContent * data );
	virtual void AppendBuffer( BufferHandle buf, const void * data_ptr, int data_size, int result );
	virtual void BeginDefaultPass( const PassAction * pass_action, int width, int height );
	virtual void BeginPass( PassHandle pass, const PassAction * pass_action );
	virtual void ApplyViewport( int x, int y, int width, int height, bool origin_top_left );
	virtual void ApplyScissorRect( int x, int y, int width, int height, bool origin_top_left );
	virtual void ApplyPipeline( PipelineHandle pip );
	virtual void ApplyBindings( const Bindings * bindings );
	virtual void ApplyUniforms( ShaderStage stage, int ub_index, const void * data, int num_bytes );
